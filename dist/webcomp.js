...
class StyleSheet {
    ...
    addRule(cssSelector, cssProperties) {
        if (this.styleSheet === undefined) {
...
function styles(host, rulesObject) {
...
class WebComponentStateTogglingValue extends State {
    togglingValue;
    constructor(name, togglingValue, value) {
        super(name, value || false);
...
function state(hostOptions, name, value, stateValue) {
    if (typeof value === "boolean" || typeof value === "undefined")
        return createSimpleState(hostOptions, name, value);
    if (typeof value === "object")
        return createTogglingValueState(hostOptions, name, value, stateValue);
    throw new Error("Invalid arguments.");
}
function createSimpleState(hostOptions, name, value) {
    if (typeof name !== "string")
        throw new Error("Invalid argument, 'name' must be a string.");
    let state = hostOptions.stateMap.get(name);
    if (!state) {
        state = new WebComponentState(name, value || false);
        hostOptions.stateMap.set(name, state);
    }
    return state;
}
function createTogglingValueState(hostOptions, name, value, stateValue) {
    let stateName;
    if (typeof name === "string")
        stateName = name;
    ...
    let state = new WebComponentStateTogglingValue(stateName, value, stateValue);
    stateSet.add(state);
    return state;
}
...
function property(hostOptions, name, value) {
...
function webComponentCallback(hostOptions, name, value) {
...
function reference(hostOptions, name) {
...
class VirtualNode {
    ...
    setAttributeBinding(attributeName, binding) {
...
function bindNodeClassListBindings(component, node, virtualNode) {
    if (!(virtualNode instanceof VirtualElement))
        return;
    for (const dataBinding of virtualNode.classListBindings) {
        if (dataBinding instanceof WebComponentStateTogglingValue) {
            component.watchState(dataBinding.getName(), nodeClassListStateBinding.bind(null, node, dataBinding.getTogglingValue()));
            continue;
        }
        if (dataBinding instanceof WebComponentProperty) {
            component.watchProperty(dataBinding.getName(), nodeClassListPropertyBinding.bind(null, node, dataBinding, { oldValue: null }));
            continue;
        }
    }
}
function nodeClassListStateBinding(node, togglingValue, newValue) {
...
function nodeClassListPropertyBinding(node, property, storage, newValue) {
    if (!storage.oldValue) {
...
function bindComponentWithNodeBindings(component, node, bindings) {
    for (const [name, value] of bindings) {
        ...
        if (value instanceof WebComponentCallback) {
            ...
            node[name] = callback.getValue().bind(component);
            continue;
        }
...
function componentPropertyBinding(node, propertyName, newValue) {
    node[propertyName] = newValue;
}
function componentStateBinding(node, propertyName, newValue) {
...
function componentStateTogglingValueBinding(node, propertyName, togglingValue, newValue) {
...
class VirtualElement extends VirtualNode {
    ...
    addClassListBinding(dataBinding) {
        this.classListBindingsMap.add(dataBinding);
    }
    get classListBindings() {
...
function setElementAttributes(element, attributeName, attributeValue, virtualElement) {
    if (attributeName.indexOf("on") === 0) {
        virtualElement.addEventListener(attributeName.toLowerCase(), attributeValue);
        return;
    }
...
function setElementAttributeAsString(element, attributeName, attributeValue, virtualElement) {
    if (attributeName === "className") {
        element.className = attributeValue;
        return;
    }
    if (attributeName === "id")
        virtualElement.setAttributeBinding("id", new WebComponentReference(attributeValue, element));
    element[attributeName] = attributeValue;
}
function setElementAttributeAsWebComponent(element, attributeName, attributeValue, virtualElement) {
    if (attributeName === "display" && attributeValue instanceof WebComponentState) {
        const state = attributeValue;
        if (state.isOff())
            element.style.display = "none";
    }
    virtualElement.setAttributeBinding(attributeName, attributeValue);
}
function setElementClassNameAsArray(element, attributeValue, virtualElement) {
    for (const className of attributeValue) {
        ...
        if (className instanceof WebComponentStateTogglingValue) {
            virtualElement.addClassListBinding(className);
            let returnedValue = className.getReturnValue();
            if (returnedValue)
                element.classList.add(returnedValue);
            continue;
        }
        if (className instanceof WebComponentProperty)
            virtualElement.setAttributeBinding("classList", className);
    }
}
function addElementChild(child, virtualElement) {
...
function template(host, rootNode) {
...
export class WebComponent {
    ...
    register() {
        customElements.define(this.name, createCustomElementClass(this.options));
    }
    _getProperty(name) {
...
function createCustomElementClass(hostOptions) {
    let template = hostOptions.template;
    let styles = hostOptions.styles;
    let propertyNameList = [...hostOptions.propertyMap.keys()].map(name => name.toLowerCase());
    const elementConstructor = class extends HTMLElement {
        propertyMap = new Map();
        stateMap = new Map();
        callbackMap = new Map();
        referenceMap = new Map();
        static observedAttributes = propertyNameList;
        constructor() {
            super();
            for (const [propertyName, componentProperty] of hostOptions.propertyMap)
                this.propertyMap.set(propertyName.toLowerCase(), new Signal(componentProperty.getValue()));
            for (const [propertyName, state] of hostOptions.stateMap)
                this.stateMap.set(propertyName, new Signal(state.getValue()));
            for (const [propertyName, callback] of hostOptions.callbackMap)
                this.callbackMap.set(propertyName, new Signal(callback.getValue()));
            for (const [propertyName, propertyList] of hostOptions.callbackWatcherMap) {
                for (const callback of propertyList)
                    this.propertyMap.get(propertyName)?.effect(callback.bind(this));
            }
            template.initialize(this);
            for (const [propertyName, reference] of hostOptions.referenceMap) {
                this.referenceMap.set(propertyName, new Signal(reference.getValue()));
            }
            hostOptions = null;
            styles.attach();
        }
        ...
        updatePropertyValue(name, callback) {
            const property = this._getPropertySignal(name);
            property.update(callback);
        }
        _getStateSignal(name) {
...
//# sourceMappingURL=webcomp.js.map